...
export const ListCharacterItem = (props) => {
    ...
    return (
        <Pressable style={globalStyles.card} onPress={() => navigation.navigate("Menu de Personagem", {character: character})}>
            <View style={globalStyles.column}>
                <Text style={globalStyles.text}>Nome do Personagem: {character.name}</Text>
                <View style={globalStyles.row}>
                <Text style={globalStyles.text}>Nível: </Text>
                <Text style={[globalStyles.text, {color: 'red'}]}>{character.level}</Text>
